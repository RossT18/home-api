...
def get_plant_water_history(plant_id: int, db: sqlite3.Connection) -> list[str]:
    rows = db.execute("SELECT date FROM water_plant_events WHERE plantId = ?", (plant_id,)).fetchall()
...
def create_plant(payload: Plant, db: DatabaseConnectionDep):
    cursor = db.execute(
        ...
        (payload.name, payload.iconName, payload.purchaseDate, payload.waterFrequency, payload.lightConditions, payload.delayUntil),
    )
    db.commit()
    row = db.execute("SELECT * FROM plants WHERE id = ?", (cursor.lastrowid,)).fetchone()
...
def update_plant(
    plant_id: int, payload: PartialPlant, db: DatabaseConnectionDep
):
...
def delete_plant(plant_id: int, db: DatabaseConnectionDep):
    db.execute("DELETE FROM water_plant_events WHERE plantId = ?", (plant_id,))
...
def water_plants(plant_ids: str, water_event: WaterEvent, db: DatabaseConnectionDep):
    ...
    for plant_id in valid_plant_ids:
        db.execute(
            "INSERT INTO water_plant_events (plantId, date) VALUES (?, ?)",
            (plant_id, date),
        )
    db.commit()

@router.post('/delay/{plant_ids}')
def delay_plant_water(plant_ids: str, delay_event: DelayEvent, db: DatabaseConnectionDep):
    plant_ids_list = [plant_id.strip() for plant_id in plant_ids.split(',')]

    plant_ids_placeholder = ','.join('?' * len(plant_ids_list))

    delay_until_date = convert_ISO_to_dt(delay_event.date) + timedelta(days=delay_event.days)
