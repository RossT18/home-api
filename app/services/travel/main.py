...
def get_directions_response(
    transit_mode: TravelMode, origin: Point, destination: Point
):
    gmaps_api_key = os.getenv("GOOGLE_MAPS_API_KEY")
    if gmaps_api_key is None:
        raise HTTPException(
            status_code=500, detail="Secret GOOGLE_MAPS_API_KEY could not be loaded"
        )
    ...
    if r.status_code != 200:
        raise HTTPException(
            ...
            detail=f"Error retrieving travel information. Reason: {r.reason}",
        )
...
def format_bus_directions_response(
    response, accepted_buses: List[str] = []
) -> List[BusInfo]:
    ...
    for route in routes:
        ...
        earlier_bus_already_found = (
            bus_name in buses_info and buses_info[bus_name].arrival_time < arrival_time
        )
