...
def get_weather_type_from_code(code: int) -> WeatherType:
    ...
    return (
        codes[code]
        if code in codes
        else WeatherType(name="Weather Unavailable", icon=get_icon_url(""))
    )
...
def get_weather_response(location: Point):
    ...
    if r.status_code != 200:
        raise HTTPException(
            ...
            detail=f"Error retrieving weather information. Reason: {r.reason}",
        )
    return r.json()
...
def format_weather_response(data) -> Weather:
    def format_time(timestamp):
        return datetime.fromtimestamp(timestamp, ZoneInfo(data["timezone"])).strftime(
            "%H:%M"
        )
